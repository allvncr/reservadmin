...
export const useReservationStore = defineStore('reservation', {
  ...
  actions: {
    ...
    async confirmReservation(reservationID) {
      try {
        await confirmReservation(useAuthStore().token, reservationID)
      } catch (err) {
        this.error =
          err.response?.data?.message ||
          'Échec lors de la confirmation de la reservation. Essayez à nouveau.'
      }
    },

    async fetchAllReservations() {
      try {
        const response = await getAllReservations(useAuthStore().token)
        ...
        this.error =
          err.response?.data?.message ||
          'Échec de la recuperation de toutes les reservations. Essayez à nouveau.'
      }
    },
  },
})
