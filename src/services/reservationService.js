...
export const confirmReservation = async (token, reservationID) => {
  return await api.put(`/reservations/${reservationID}/confirm`, null, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
...
export const getAllReservations = async (token) => {
  return await api.get('/reservations/admin/all', {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
